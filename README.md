...
Panel Kiri (Setup & Pengaturan)
    ...
    Aspect Ratio: Centang rasio layar target. Anda bisa mencentang lebih dari satu untuk melakukan batch rendering.
...
Panel Kanan (Eksekusi)
    ...
    Output Settings: Tentukan nama dasar file hasil (contoh: podcast_eps1.mp4). Sistem akan otomatis menambahkan label resolusi di nama file akhirnya (contoh: podcast_eps1_9x16_Podcast.mp4).

    Klik tombol biru 🚀 START BATCH RENDER.

    Pantau seluruh alur proses download gambar AI dan kompresi video melalui Live Process Terminal di bagian bawah.

⚙️ Arsitektur Core (Under the Hood)

    GUI Framework: Menggunakan PySide6 untuk rendering UI berbasis komponen (Qt). Tampilan diformat ulang secara menyeluruh dengan CSS khusus agar menyamai software editing PC komersial.
    ...
    Terminal Redirector: Membajak jalur komunikasi stdout (print) dari Python dan meneruskannya secara asinkron ke QTextEdit GUI menggunakan Signal dan Slot.

    FFmpeg Filter Logic: Jantung pemrosesan visual. Memanfaatkan imageio-ffmpeg untuk memanipulasi filter_complex berlapis yang menggabungkan: Video asli, Background AI, Scaling dinamis, Teks/Gambar Watermark, dan pemetaan Audio murni dalam satu perintah CLI komprehensif.

⚠️ Disclaimer

    Pastikan Anda tidak mengekspos API Key OpenAI rahasia Anda.

    Penggunaan API DALL-E 3 akan memotong saldo kredit di akun OpenAI Anda sesuai tarif resmi yang berlaku. Jika tidak ingin menggunakan fitur berbayar ini, kosongkan kolom API Key pada aplikasi.
